...
//         }
     ...
     /**
      * return an array of urls to all images rendered as full image for the given imageInfo object
      */
     imageView.Image.getFullLayers = function(imageInfo) {
         let baseUrl = imageInfo["@id"];
         let urls = [];
         if(imageInfo.tiles) {
             imageInfo.tiles.forEach( tile => {
                 let tileWidth = tile.width;
                 if(tile.scaleFactors) {                     
                     tile.scaleFactors.forEach(scale => {
                         if(imageInfo.width/scale <= tileWidth) {
                             let width = Math.ceil(imageInfo.width/scale);
                             let url = baseUrl + "/full/" + width + ",/0/default.jpg";
                             urls.push(url);
                         } 
                     })
                 }
