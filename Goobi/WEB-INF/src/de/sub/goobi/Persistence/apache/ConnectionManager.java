...
public class ConnectionManager {
    ...
    public ConnectionManager(SqlConfiguration config) {
        try {
            if (ConfigMain.getBooleanParameter("DatabaseUseJNDI", false)) {
                connectToDB();
            } else {
                connectToDB(config);
            }
    ...
    private void connectToDB(SqlConfiguration config) {
        ...
        try {
            this.ds =
                    setupDataSource(config.getDbURI(), config.getDbUser(), config.getDbPassword(), config.getDbPoolMinSize(), config
                            .getDbPoolMaxSize());
            logger.debug("Connection attempt to database succeeded.");
        } catch (Exception e) {
            logger.error("Error when attempting to connect to DB ", e);
