...
public class Pagination {
    ...
    public static final int COUNTING_PAGINATION_ORIENTAL = 6;
     ...
     * Consturctor
     * 
     * @param allSelectedPages selected pages as string
     * @param newPaginated metadata of physical DocStrct 'BoundBook'
     * @param paginationScope scope of pagination ( all pages = 1,only selected pages = 2)
     * @param paginationType type of pagination label (arabic = 1, roman = 2, uncounted = 3)
     * @param paginationMode type of pagination (pages = 1, columns = 2, foliation = 3, foliation rectoVerso = 4, pagination rectoVerso = 5)
     * @param paginationStartValue start value
     */
    ...
    public String doPagination() {
        ...
        for (int i = firstPageNumber; i < this.newPaginated.length; i++) {
            ...
            if (this.paginationMode == COUNTING_PAGINATION_RECTOVERSO || paginationMode == COUNTING_PAGINATION_ORIENTAL) {
                if (this.paginationType.equals(PAGINATION_UNCOUNTED)) {
                    nextPaginationLabel = getRectoVersoSuffixForPagination(currentPageNumber);
                } else {
                    nextPaginationLabel += getRectoVersoSuffixForPagination(currentPageNumber);
    ...
    private double getNextPageNumber(double currentPageNumber) {
        switch (this.paginationMode) {
            ...
            case COUNTING_PAGINATION_ORIENTAL:
