...
public class FilesystemHelper {
    ...
    public static void createDirectory(String dirName) throws IOException, InterruptedException {
        if (ConfigurationHelper.getInstance().useS3() && S3FileUtils.isPathOnS3(Paths.get(dirName))) {
            return;
        }
    ...
    public static boolean isOcrFileExists(Process inProcess, String ocrFile) {
        try {
            if (!ConfigurationHelper.getInstance().useS3()) {
                ...
                File altoXml = new File(inProcess.getOcrAltoDirectory(), ocrFile + ".xml");
                File alto = new File(inProcess.getOcrAltoDirectory(), ocrFile + ".alto");
                return (txt.exists() && txt.canRead()) || (xml.exists() && xml.canRead()) || (altoXml.exists() && altoXml.canRead())
                        || (alto.exists() && alto.canRead());
            } else {
                Path txt = Paths.get(inProcess.getOcrTxtDirectory(), ocrFile + ".txt");
                Path xml = Paths.get(inProcess.getOcrXmlDirectory(), ocrFile + ".xml");
                Path altoXml = Paths.get(inProcess.getOcrAltoDirectory(), ocrFile + ".xml");
                Path alto = Paths.get(inProcess.getOcrAltoDirectory(), ocrFile + ".alto");
                StorageProviderInterface sp = StorageProvider.getInstance();
                return sp.isFileExists(xml) || sp.isFileExists(txt) || sp.isFileExists(altoXml) || sp.isFileExists(alto);
            }
