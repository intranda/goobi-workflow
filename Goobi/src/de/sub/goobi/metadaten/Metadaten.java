...
public class Metadaten {
	...
	private void MetadatenalsBeanSpeichern(DocStruct inStrukturelement) {
		...
		List<MetadataGroup> groups = this.metahelper.getMetadataGroupsInclDefaultDisplay(inStrukturelement,
				(String) Helper.getManagedBeanValue("#{LoginForm.myBenutzer.metadatenSprache}"), this.myProzess);
		if (groups != null) {
	...
	private TreeNodeStruct3 buildTree(TreeNodeStruct3 inTree, DocStruct inLogicalTopStruct, boolean expandAll) {
		 ...
		 */
		String label = inLogicalTopStruct.getType()
				.getNameByLanguage((String) Helper.getManagedBeanValue("#{LoginForm.myBenutzer.metadatenSprache}"));
		if (label == null) {
		...
		if (docstructName != null && (oldDocstructName.isEmpty() || !oldDocstructName.equals(docstructName))) {
			...
			if (docstructName != null) {
				...
				try {
					...
					List<? extends Metadata> myTempMetadata = this.metahelper.getMetadataInclDefaultDisplay(ds,
							(String) Helper.getManagedBeanValue("#{LoginForm.myBenutzer.metadatenSprache}"), false, this.myProzess);
					if (myTempMetadata != null) {
						for (Metadata metadata : myTempMetadata) {
							MetadatumImpl meta = new MetadatumImpl(metadata, 0, this.myPrefs, this.myProzess);
	...
	private String createImageUrl(Image currentImage, Integer size, String format, String baseUrl) {
		...
		url.append("/cs").append("?action=").append("image").append("&format=").append(format).append("&sourcepath=")
				.append("file://" + imageFolderName + currentImage.getImageName()).append("&width=").append(size).append("&height=").append(size);
		return url.toString();
	}
