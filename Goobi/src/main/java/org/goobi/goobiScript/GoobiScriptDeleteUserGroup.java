...
public class GoobiScriptDeleteUserGroup extends AbstractIGoobiScript implements IGoobiScript {
	...
	public String getSampleCall() {
		...
		addNewActionToSampleCall(sb,
				"This GoobiScript allows to remove a user group from a workflow step where it was assigned to.");
		addParameterToSampleCall(sb, "steptitle", "Scanning", "Title of the workflow step to adapt");
		addParameterToSampleCall(sb, "group", "Photographers",
				"Name of the user group that shall be removed from the workflow step");
		return sb.toString();
		...
		for (Iterator<Step> iterator = p.getSchritteList().iterator(); iterator.hasNext();) {
			...
			if (s.getTitel().equals(parameters.get("steptitle"))) {
				...
				if (myBenutzergruppe.contains(myGroup)) {
					...
					Helper.addMessageToProcessLog(p.getId(), LogType.DEBUG, "Deleted usergroup '" + myGroup.getTitel()
							+ "' from step '" + s.getTitel() + "' using GoobiScript.", username);
					log.info("Deleted usergroup '" + myGroup.getTitel() + "' from step '" + s.getTitel()
							+ "' using GoobiScript for process with ID " + p.getId());
					gsr.setResultMessage("Deleted usergroup '" + myGroup.getTitel() + "' from step '" + s.getTitel()
							+ "' successfully.");
	...
	private Usergroup getUserGroup(String UserGroupTitle) {
		Usergroup myGroup;
		try {
			List<Usergroup> treffer = UsergroupManager.getUsergroups(null, "titel='" + UserGroupTitle + "'", null, null,
					null);
