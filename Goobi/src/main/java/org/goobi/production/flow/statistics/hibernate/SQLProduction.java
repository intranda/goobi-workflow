...
/************************************************************************
 * 
 * This Class provide methods to prepare SQL Queries needed for production statistics. It depends on time frame and time unit.
 * 
...
class SQLProduction extends SQLGenerator {
    ...
    public String getSQL() {
        ...
        subQuery = "(SELECT prozesse.prozesseid AS singleProcess, " + "prozesse.sortHelperImages AS pages, "
                + getIntervallExpression(myTimeUnit, "BearbeitungsEnde") + " " + "AS intervall, BearbeitungsEnde AS timeLimiter "
                + "FROM  schritte inner join prozesse on schritte.prozesseid=prozesse.prozesseid " + "WHERE " + innerWhereClause
                + "GROUP BY prozesse.prozesseid) AS table_1";

        mySql = "SELECT count(table_1.singleProcess) AS volumes, " + "sum(table_1.pages) AS pages, table_1.intervall " + "FROM " + subQuery + " "
                + outerWhereClause + " GROUP BY intervall " + "ORDER BY timeLimiter";
    ...
    public String getSQL(Integer stepDone) {
        ...
        String innerWhereClause = "schritte.reihenfolge=" + stepDone.toString() + " AND " + "schritte.bearbeitungsstatus>2";
