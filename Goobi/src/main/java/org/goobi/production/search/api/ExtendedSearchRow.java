...
public class ExtendedSearchRow implements Serializable {
    ...
    public String createSearchString() {
        String value = "";
        String notSelected = Helper.getTranslation("notSelected");
        ...
        } else if (fieldName.equals("PROJECT") && !this.projectName.equals(notSelected)) {
            value = this.fieldOperand + FilterString.PROJECT + this.projectName;

        } else if (fieldName.equals("METADATA") && !this.metadataName.equals(notSelected) && !metadataValue.isEmpty()) {
            value = this.fieldOperand + FilterString.METADATA + metadataName + ":" + metadataValue;

        } else if (fieldName.equals("PROCESSPROPERTY") && !processPropertyName.equals(notSelected) && !processPropertyValue.isEmpty()) {
            value = this.fieldOperand + FilterString.PROCESSPROPERTY + this.processPropertyName + ":" + processPropertyValue;

        } else if (fieldName.equals("WORKPIECE") && !masterpiecePropertyName.equals(notSelected) && !masterpiecePropertyValue.isEmpty()) {
            value = this.fieldOperand + FilterString.WORKPIECE + this.masterpiecePropertyName + ":" + masterpiecePropertyValue;

        } else if (fieldName.equals("TEMPLATE") && !templatePropertyName.equals(notSelected) && !templatePropertyValue.isEmpty()) {
            value = this.fieldOperand + FilterString.TEMPLATE + this.templatePropertyName + ":" + templatePropertyValue;

        } else if (fieldName.equals("STEP") && !stepStatus.equals(notSelected) && !stepName.isEmpty() && !stepName.equals(notSelected)) {
