...
@Log4j2
public class GoobiScriptSetStepStatus extends AbstractIGoobiScript {
    ...
    public List<GoobiScriptResult> prepare(List<Integer> processes, String command, Map<String, String> parameters) {
        ...
        if (status == null || "".equals(status)) {
            Helper.setFehlerMeldung(missingParameter, STATUS);
            return new ArrayList<>();
        }
        ...
        if (!"0".equals(status) && !"1".equals(status) && !"2".equals(status) && !"3".equals(status) && !"4".equals(status) && !"5".equals(status)) {
            ...
            options.append(STATUS_NUMBER_ERROR + "=" + STATUS_TEXT_ERROR + ", ");
            options.append(STATUS_NUMBER_DEACTIVATED + "=" + STATUS_TEXT_DEACTIVATED);
            Helper.setFehlerMeldung("Wrong status parameter: status ", "(possible: " + options.toString() + ")");
            return new ArrayList<>();
        }
