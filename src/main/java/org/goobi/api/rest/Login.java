...
public class Login {
    ...
    @ApiResponse(responseCode = "500", description = "Internal error")
    @Tag(name = "login")
    public void openIdLoginAuthFlow(@FormParam("error") String error, @FormParam("code") String authCode, @FormParam("state") String authState)
            throws IOException {
        ConfigurationHelper config = ConfigurationHelper.getInstance();
        String clientID = config.getOIDCClientID();
        String nonce = (String) servletRequest.getSession().getAttribute("openIDNonce");
            ...
            if (state.equals(authState)) {
                ...
                if (authCode != null) {
                    ...
                    try {
                        ...
                        bodyMap.add(new BasicHeader("redirect_uri", servletRequest.getScheme() + "://" + servletRequest.getServerName() + ":"
                                + servletRequest.getServerPort() + applicationPath + "/api/login/openid/authorizationcodeflow"));
