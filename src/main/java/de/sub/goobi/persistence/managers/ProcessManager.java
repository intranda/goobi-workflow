...
public class ProcessManager implements IManager {
    ...
    public static int countProcessTitle(String title, Institution institution) {
        try {
            return ProcessMysqlHelper.getProcessCount(" WHERE prozesse.titel = '" + MySQLHelper.escapeSql(title) + "'", institution);
        } catch (SQLException e) {
            log.error(e);
        }
        return 0;
    ...
    public static ResultSetHandler<List<Process>> resultSetToProcessListHandler = new ResultSetHandler<>() {
        ...
        public List<Process> handle(ResultSet rs) throws SQLException {
            ...
            try {
                while (rs.next()) {
                    answer.add(convert(rs)); // implies that o != null
                }
            } finally {
                rs.close();
            }
    ...
    /**
     * Maps a row of the prozesse table to a {@link Process}. This is a pure row mapper: it reads only the columns of the process table and stores the
     * foreign keys of the associated objects (ruleset, batch, docket), which - together with the journal - are then resolved lazily on first access by
     * the corresponding getters ({@link Process#getRegelsatz()}, {@link Process#getBatch()}, {@link Process#getDocket()}, {@link Process#getJournal()}).
     * It must NOT acquire any further database connection here, because it runs while the connection of the surrounding process query is still open.
     * Nested connection acquisition would, under concurrent load (e.g. the IIIF image API during rapid image navigation in the METS editor), exhaust
     * the connection pool and deadlock the application.
     */
    private static Process convert(ResultSet rs) throws SQLException {
        ...
        // Store only the foreign keys of the associated objects. They are resolved lazily by the corresponding getters,
        // each using its own connection AFTER this query has been closed (no nested connection acquisition here).
        p.setMetadatenKonfigurationID(rs.getInt("MetadatenKonfigurationID"));
        p.setDocketId(rs.getInt("docketID"));
        int batchID = rs.getInt("batchID");
        if (!rs.wasNull()) {
            p.setBatchId(batchID);
        }
        p.markJournalForLazyLoading();
        return p;
    }
