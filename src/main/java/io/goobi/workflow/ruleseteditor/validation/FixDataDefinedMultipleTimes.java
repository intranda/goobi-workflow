...
public class FixDataDefinedMultipleTimes {
	/**
		...
		if (toRemove != null) {
			...
			if (parent != null) {
                ...
                if (grandParent != null) {
                    ...
                    // Remove preceding whitespace text node, if any
                    if (index > 0) {
                        org.jdom2.Content previous = grandParent.getContent(index - 1);
                        if (previous instanceof org.jdom2.Text) {
                            org.jdom2.Text text = (org.jdom2.Text) previous;
                            if (text.getTextTrim().isEmpty()) {
                                grandParent.removeContent(previous);
                            }
                        }
                    }
