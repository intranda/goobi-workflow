...
const initPlayer = () => {
    ...
    const audioElement = document.getElementById('videoplayer'); // Keep same ID for compatibility
    if (!audioElement) {
        console.warn('Audio element #videoplayer not found');
        return;
    }

    audioElement.style.setProperty('--plyr-color-main', 'var(--clr-primary, #368ee0)');

    try {
        const player = new Plyr('#videoplayer', {
            settings: ['speed'], // Remove captions since audio doesn't support them natively
            invertTime: false,
    ...
    timestampButtons.forEach(button => {
        button.addEventListener('click', (event) => {
            ...
            if (isNaN(currentTime)) {
                console.warn('Unable to get current audio time');
                return;
            }
...
const reinitTimestampButtons = () => {
    ...
    if (audioElement && audioElement.plyr) {
        initTimestampButtons(audioElement.plyr);
    }
};
