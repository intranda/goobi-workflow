...
public class SessionForm {
	...
	private void sessionsAufraeumen(int time) {
		...
		for (Iterator iter = temp.iterator(); iter.hasNext();) {
			...
			long differenz = System.currentTimeMillis()
					- ((Long) map.get("last2")).longValue();
			// logger.debug(element.get("last2") + ": " +
			// String.valueOf(differenz / 1000));
			if (differenz / 1000 > time || map.get("address") == null
					|| (map.get("user").equals("- ausgeloggt - "))) {
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public void sessionBenutzerAktualisieren(HttpSession insession,
			Benutzer inBenutzer) {
		...
		for (Iterator iter = this.alleSessions.iterator(); iter.hasNext();) {
			...
			if (map.get("id").equals(insession.getId())) {
				if (inBenutzer != null) {
					...
					insession.setMaxInactiveInterval(inBenutzer
							.getSessiontimeout());
				} else {
	...
	@SuppressWarnings("rawtypes")
	public boolean BenutzerInAndererSessionAktiv(HttpSession insession,
			Benutzer inBenutzer) {
		boolean rueckgabe = false;
		for (Iterator iter = this.alleSessions.iterator(); iter.hasNext();) {
			HashMap map = (HashMap) iter.next();
			boolean sessiongleich = map.get("id").equals(insession.getId());
			boolean nutzergleich = inBenutzer.getId().intValue() == ((Integer) map
					.get("userid")).intValue();
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public void alteSessionsDesSelbenBenutzersAufraeumen(HttpSession inSession,
			Benutzer inBenutzer) {
